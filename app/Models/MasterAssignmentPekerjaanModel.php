...
class MasterAssignmentPekerjaanModel extends Model
{
    ...
    protected $allowedFields = ['kdes3', 'kdes4', 'satuan', 'kdkerjaan', 'kuantitastarget', 'kuantitasrealisasi', 'wakturealisasi', 'kdsatker', 'created_by', 'id_pekerjaan', 'assign','bukti','statuspenyelesaiiantarget','statuspenyelesaiianwaktu','statusverifikasi'];
    ...
    public function upsertData($data)
    {
        ...
        $primaryKeyColumns = ['id_pekerjaan','kdsatker'];
    ...
    public function countByKab($year, $month)
    {
        $subquery = "(SELECT kdsatker, COUNT(tbl_pekerjaan_child.kdsatker) as count, created_at 
        FROM tbl_pekerjaan_child 
        WHERE YEAR(created_at) = '$year' AND MONTH(created_at) = '$month' 
        GROUP BY kdsatker) as a";
        
        $builder = $this->db->table('mst_satker');
        $builder->select('kd_satker, count');
        $builder->join($subquery, 'a.kdsatker = mst_satker.kd_satker', 'left');
    ...
    public function rankByKab($year, $month)
    {
        $subquery = "(SELECT kdsatker, AVG(tbl_pekerjaan_child.skor) as average_score, created_at 
        FROM tbl_pekerjaan_child 
        WHERE YEAR(created_at) = '$year' AND MONTH(created_at) = '$month' 
        GROUP BY kdsatker) as a";
        ...
        $builder = $this->db->table('mst_satker');
        $builder->select('kd_satker, average_score');
        $builder->join($subquery, 'a.kdsatker = mst_satker.kd_satker', 'left');
        $builder->groupBy('kd_satker');
    ...
    public function rankByFungsi($year, $month)
    {
        $subquery = "(SELECT kdes4, AVG(tbl_pekerjaan_child.skor) as average_score, created_at 
        FROM tbl_pekerjaan_child 
        WHERE YEAR(created_at) = '$year' AND MONTH(created_at) = '$month' 
        GROUP BY kdes4) as a";

        $builder = $this->db->table('mst_es4');
        $builder->select('kd_es4, average_score');
        $builder->join($subquery,'a.kdes4 = mst_es4.kd_es4', 'left');
        $builder->where("LENGTH(kd_es4)", 2);
        $builder->groupBy('kd_es4');
    ...
    public function rankByKabAll()
    {
        $builder = $this->db->table('mst_satker');
        $builder->select('kd_satker, AVG(tbl_pekerjaan_child.skor) as average_score');
    ...
    public function rankByFungsiAll()
    {
        $builder = $this->db->table('mst_es4');
        $builder->select('kd_es4, AVG(tbl_pekerjaan_child.skor) as average_score');
        $builder->join('tbl_pekerjaan_child', 'tbl_pekerjaan_child.kdes4 = mst_es4.kd_es4', 'left');
        $builder->where("LENGTH(kd_es4)", 2);
        $builder->groupBy('kd_es4');
