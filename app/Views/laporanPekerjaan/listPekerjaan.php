                                        ...
                                        foreach ($list_full_laporan_harian as $list) : ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <form action="<?= base_url('/verifikasi/insert') ?>" method="POST">
                                                            <input type="hidden" name="id" value="<?= $list['id']; ?>">
                                                                <?php if ($list['statusverifikasi'] == '1') : ?>
                                                                    <span class="text-success">Accepted</span>
                                                                    <!-- <button type="submit" name="verifikasi" value="edit" class="btn btn-primary"> <i class="fas fa-sync-alt"></i></button> -->
                                                                <?php elseif ($list['statusverifikasi'] == '2') : ?>
                                                                    <span class="text-danger">Rejected</span>
                                                                <?php elseif ($list['kuantitasrealisasi'] == '') : ?>
                                                                    <span class="text-muted">No Effort</span>
                                                                <?php elseif ($list['statusverifikasi'] == '') : ?>
                                                                    <i><span class="text-primary">Waiting...</span></i>
                                                                    <!-- <button type="submit" name="verifikasi" value="edit" class="btn btn-primary"> <i class="fas fa-sync-alt"></i></button> -->
                                                                <?php elseif ($list['statusverifikasi'] == '0') : ?>
                                                                    <i><span class="text-primary">Waiting...</span></i>
                                                                    <!-- <button type="submit" name="verifikasi" value="edit" class="btn btn-primary"> <i class="fas fa-sync-alt"></i></button> -->
                                                                <?php endif; ?>
                                                </td>
                                                ...
                                                <td class="d-flex justify-content-center">
                                                    <form action="<?= base_url('/penyelesaian/insert') ?>" method="POST">
                                                            <input type="hidden" name="id" value="<?= $list['id']; ?>">
                                                                <?php if ($list['statuspenyelesaian'] == '1') : ?>
                                                                    <span class="text-success">Selesai</span>
                                                                    <button type="submit" name="statuspenyelesaian" value="0" class="btn btn-primary"> <i class="fas fa-sync-alt"></i></button>
                                                                <?php else : ?>
                                                                    <button type="submit" name="statuspenyelesaian" value="1" class="btn btn-success">Selesai</button>
                                                                <?php endif; ?> 
                                                    </form>
                                                </td>
                                                <?php endif ?>
...
<!-- Tombol tambah pekerjaan hanya ditampilkan jika pengguna bukan dengan es3=98 -->
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <?php if ($laporan_harian_tertentu != NULL) : ?>
                                        ...
                                        <?php
                                        for ($i = 0; $i < count($list_uraian = $laporan_harian_tertentu); $i++): ?>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php if ($list_uraian[$i]['bukti']): ?>
                                                        <a href="<?= base_url($list_uraian[$i]['bukti']); ?>" target="_blank">
                                                        Bukti Realisasi
                                                        </a>
                                                        <!-- <a href="<?= 'https://' . $list_uraian[$i]['bukti']; ?>" target = "_blank">
                                                        <img src="<?= base_url('bukti/' . $list_uraian[$i]['bukti']); ?>" alt="Bukti Realisasi"> -->
                                                        <!--<img src="<?= 'https://' . $list_uraian[$i]['bukti']; ?>" alt="Bukti Realisasi"> </a>
                                                        <a href = "<?= 'https://'. $list_uraian[$i]['bukti']; ?>" target = "_blank"><?= $list_uraian[$i]['bukti']; ?></a> 
                                                        <a href="<?= 'https://' . $list_uraian[$i]['bukti']; ?>" target="_blank">
                                                        <img src="<?= 'https://' . $list_uraian[$i]['bukti']; ?>" alt="Bukti Realisasi"> </a>
                                                        <a href="<?= 'https://' . $list_uraian[$i]['bukti']; ?>" download><?= $list_uraian[$i]['bukti']; ?></a> -->
                                                <?php endif; ?>
                                                </td>    
                                                </td>
                                                <td>
                                                    <form action="<?= base_url('/verifikasi/insert') ?>" method="POST">
                                                        <input type="hidden" name="kdsatker" value="<?= $list_uraian[$i]['kdsatker']; ?>">
                                                        <input type="hidden" name="id_pekerjaan" value="<?= $list_uraian[$i]['id']; ?>">
                                                            <?php if ($list_uraian[$i]['statusverifikasi'] == '1') : ?>
                                                                <span class="text-success">Accepted</span>
                                                                <button type="submit" name="statusverifikasi" value="0" class="btn btn-primary"> <i class="fas fa-sync-alt"></i></button>
                                                            <?php elseif ($list_uraian[$i]['statusverifikasi'] == '2') : ?>
                                                                <span class="text-danger">Rejected</span>
                                                                <button type="submit" name="statusverifikasi" value="0" class="btn btn-primary"> <i class="fas fa-sync-alt"></i></button>
                                                            <?php else : ?>
                                                                <button type="submit" name="statusverifikasi" value="1" class="btn btn-success">Accept</button>
                                                                <button type="submit" name="statusverifikasi" value="2" class="btn btn-danger">Reject</button>
                                                            <?php endif; ?> 
                                                    </form>
                                                </td>
                                            </tr>
        ...
        <form action="<?= base_url('/updateRealisasiPekerjaan'); ?>" method="POST" class="modal-content" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    ...
                    <div class="form-group text-center col-6">
                        <div class="password">
                            <input id="dateInput" type="date" class="form-control  w-100" name="field_waktu_realisasi" rows="1" placeholder="Pilih Tanggal" max="" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="form-group text-left col-6">
                        <div class="password">
                            <input type="file" accept=".png, .jpg, .pdf" class="form-control w-100" name="field_bukti_realisasi" required>
                            <?php if (isset($_FILES['field_bukti_realisasi'])): ?>
                                <?php
                                $allowedExtensions = ['png', 'jpg', 'pdf'];
                                $fileExtension = strtolower(pathinfo($_FILES['field_bukti_realisasi']['name'], PATHINFO_EXTENSION));

                                if (!in_array($fileExtension, $allowedExtensions)) {
                                    echo "<p class='text-danger'>File type not allowed. Please upload a PNG or JPG file.</p>";
                                } else {
                                    echo "<p>" . $_FILES['field_bukti_realisasi']['name'] . "</p>";
                                }
                                ?>
                            <?php endif; ?>
                        </div>
...
<script>
    ...
    $('#tabelData tbody').on('click', '#del-pekerjaan', function(e) {
        e.preventDefault();
        // Show a confirmation prompt
        Swal.fire({
            title: 'Apakah kamu yakin?',
            text: 'Aksi ini tidak dapat dibatalkan kembali. Data pekerjaan untuk kab/kota juga akan terhapus',
            icon: 'warning',
